...
class RequestsBase:
	...
	def execute_test_cases(self, api_info):
		...
		:param api_info: yaml里面接口信息
		:return:
		api_info:  样值
		{'baseinfo': {'api_name': '获取商品列表', 'url': '/coupApply/cms/goodList', 'method': 'get',
		             'headers': '${get_headers(data)}',
		             'cookies': {'cookie': '75hb6j35b4n766j36kk756j43436k56jk34j6'}}, 'testCase': [
			{'case': '获取商品列表', 'params': {'msgType': 'getHandsetListOfCust', 'page': 1, 'size': 20},
			 'validation': [{'code': 200}, {'eq': {'error_code': '0000'}}],
			 'extract_list': {'good_ids': '$.goodsList[*].goodsId'}}]}
		"""
		print(api_info)
		try:
			# 从api_info的baseinfo 提取 请求接口的四要素：请求地址、请求方法、请求参数、请求头、出参等
			# 获取ip地址
			...
			cookie = api_info['baseinfo'].get('cookies')
