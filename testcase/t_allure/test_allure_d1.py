...
class TestAllureDemo:
	...
	def test_03(self):
		...
		allure.attach('函数执行中', '接口入参', allure.attachment_type.TEXT)
		assert 3 == 3
	...
	def test_get_info(self, name):
		...
		with allure.step("测试步骤二"):
			print("allure学习中。。。。。")
		# allure.attach(body='人员信息校验', name=f'验证{name}',attachment_type=allure.attachment_type.JPG)
		allure.attach(body='人员信息校验', name=f'验证{name}', attachment_type=allure.attachment_type.TEXT)
